...
import extra from "./DomicileCinematicExtra.module.css";
...
const visualImages = [
  { src: "/chosen/aurelian-villa.webp", label: "RESIDENTIAL CONTEXT", title: "A home worth looking after well" },
  { src: "/chosen/casa-lumara.webp", label: "PROPERTY DETAIL", title: "Care beyond the obvious" },
  { src: "/chosen/atria-residence.webp", label: "OWNER-AWAY CARE", title: "Local presence when you are not" },
  { src: "/chosen/casa-palma.webp", label: "BUILT ENVIRONMENT", title: "A technical eye behind the care" },
];
...
export default function DomicileCinematicRich() {
  const heroRef = useRef<HTMLElement>(null);
  const reduceMotion = useReducedMotion();
  const { scrollYProgress } = useScroll({ target: heroRef, offset: ["start start", "end start"] });
  const rawScale = useTransform(scrollYProgress, [0, 1], [1, 1.085]);
  ...
  return (
    <main className={styles.page}>
      ...
      <section className={styles.hero} ref={heroRef}>
        ...
        <div className={extra.heroThumbs}>
          <motion.div className={`${extra.heroThumb} ${extra.heroThumbOne}`} animate={reduceMotion ? undefined : { y: [0, -10, 0], rotate: [-3, -1.5, -3] }} transition={{ duration: 6.4, repeat: Infinity, ease: "easeInOut" }}>
            <Image src="/domicile/properties/property-c1.webp" alt="Private residence under care" fill unoptimized sizes="180px" /><div className={extra.heroThumbShade} /><span className={extra.heroThumbLabel}>ROUTINE CARE</span>
          </motion.div>
          <motion.div className={`${extra.heroThumb} ${extra.heroThumbTwo}`} animate={reduceMotion ? undefined : { y: [0, 9, 0], rotate: [4, 2.5, 4] }} transition={{ duration: 7.1, repeat: Infinity, ease: "easeInOut" }}>
            <Image src="/domicile/properties/property-street.webp" alt="Private home under care" fill unoptimized sizes="160px" /><div className={extra.heroThumbShade} /><span className={extra.heroThumbLabel}>OWNER AWAY</span>
          </motion.div>
        </div>
      ...
      <section className={extra.imageSequence} aria-label="Residential visual context">
        <div className={extra.imageSequenceHead}><small>RESIDENTIAL VISUAL WORLD</small><h2>The kind of property care should feel worthy of.</h2><p>Additional residential imagery gives DŌMICILE a richer architectural world without presenting these images as named client properties.</p></div>
        <div className={extra.imageStack}>
          {visualImages.map((item, index) => {
            const classes = [extra.imageStackMain, extra.imageStackLeft, extra.imageStackRight, extra.imageStackMini];
            return <motion.article key={item.src} className={`${extra.imageStackCard} ${classes[index]}`} initial={{ opacity: 0, y: 48, scale: .98 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} viewport={{ once: true, margin: "-70px" }} transition={{ duration: .85, delay: index * .09, ease: [0.16, 1, 0.3, 1] }}><Image src={item.src} alt={item.title} fill unoptimized sizes="(max-width: 900px) 100vw, 50vw" /><div><small>{item.label}</small><strong>{item.title}</strong></div></motion.article>;
          })}
        </div>
      </section>
      ...
      <section className={styles.care} id="care">
        ...
        <div className={styles.careGrid}>
          ...
          <motion.article className={styles.darkCard} whileHover={{ y: -8 }}><Image src="/chosen/casa-forma.webp" alt="Residence detail" fill unoptimized /><div className={styles.cardShade} /><span>02</span><h3><TextRoll>Maintenance & repairs</TextRoll></h3><p>Issues are scoped, coordinated and followed through instead of being passed from person to person.</p></motion.article>
          ...
          <motion.article className={styles.lightCard} whileHover={{ y: -8 }}><Image src="/chosen/brick-vine.webp" alt="Residential property detail" fill unoptimized /><div className={styles.cardShade} /><span>04</span><h3><TextRoll>Urgent response</TextRoll></h3><p>Detect, contact, confirm authority, coordinate, document and report.</p></motion.article>
        </div>
      </section>
      ...
      <section className={styles.ownerView} id="owner-view">
        <div className={styles.ownerPhoto}><Image src="/domicile/properties/property-street.webp" alt="Private home under DŌMICILE care" fill unoptimized /><div><small>PROPERTY RECORD</small><strong>Kigali · Private Home</strong></div><div className={extra.ownerMiniGallery}><div className={extra.ownerMiniCard}><Image src="/chosen/casa-lumara.webp" alt="Residential context" fill unoptimized /></div><div className={extra.ownerMiniCard}><Image src="/chosen/aurelian-villa.webp" alt="Residential context" fill unoptimized /></div></div></div>
