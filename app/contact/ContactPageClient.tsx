...
export default function ContactPageClient({ content }: { content?: ContactPageContent | null }) {
  const activeInquiryTypes = content?.inquiryTypes?.length
    ? content.inquiryTypes.filter(Boolean)
    : inquiryTypes;
  const activeContactDetails = content?.contactDetails?.length
    ? content.contactDetails
        .filter((item): item is { label: string; value: string; href: string } => Boolean(item.label && item.value && item.href))
    : contactDetails;
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {content?.heroIntro || "Tell us about your project, site, ambition, timeline, and current stage. We will help define whether you need design, consultancy, site coordination, planning support, or development guidance."}
          </motion.p>
        </div>
        ...
        >
          <Image
            src={content?.heroImage?.url || "/contact-hero.png"}
            alt={content?.heroImage?.alt || "Contact IMVO"}
            fill
            priority
            style={{
              objectFit: "cover",
      ...
      >
        ...
        >
          <div style={{ position: "sticky", top: 140 }}>
            ...
            >
              {activeContactDetails.map((item, index) => (
                <motion.a
                  key={item.label}
                  href={item.href}
          ...
          >
            ...
            >
              {content?.formKicker || "Project Brief"}
            </div>
            ...
            >
              {content?.formHeading || "Submit your inquiry."}
            </h2>

            {content?.formIntro && (
              <p style={{ marginTop: 18, color: "rgba(255,255,255,0.62)", lineHeight: 1.7, maxWidth: 620 }}>
                {content.formIntro}
              </p>
            )}
                ...
                {!isSuccess ? (
                  ...
                  >
                    ...
                    >
                      {isSubmitting
                        ? "TRANSMITTING BRIEF..."
                        : content?.submitLabel || "SUBMIT INQUIRY ↗"}
                    </button>
                  </motion.form>
                ) : (
                  <motion.div
                  ...
                  >
                    ...
                    >
                      {content?.successKicker || "Inquiry Submitted"}
                    </div>
                    ...
                    >
                      {content?.successHeading || "Brief received."}
                    </h3>
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {content?.locationKicker || "Location"}
            </div>
            ...
            >
              <span style={{ whiteSpace: "pre-line" }}>
                {content?.locationHeading || "Kigali base.\nRegional reach."}
              </span>
            </h2>
            ...
            >
              {content?.locationText || "IMVO Group operates from Kigali, Rwanda, supporting design, consultancy, site coordination, planning, and development-oriented projects across Rwanda and the wider East African region."}
            </p>
          </motion.div>
        </div>
        ...
        >
          <iframe
            title="IMVO Location"
            src={content?.mapUrl || "https://maps.google.com/maps?q=-1.9235606,30.0709587&z=18&output=embed"}
            width="100%"
            height="100%"
            loading="lazy"
            style={{
